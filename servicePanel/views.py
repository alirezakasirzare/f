...
def servicepanel_page(request):
    if request.user.is_authenticated:
        ...
        if user.status == True:
            ...
            return render(request,'ServicePanel/servicepanel_page/servicepanel_page.html',context)
        else:
...
def servicepanel_chatlist_page(request):
    if request.user.is_authenticated:
        user = Userperson.objects.filter(id=request.user.id).first()
        if user.status == True:
            site_settings = SiteSettings.objects.last()
            ...
            return render(request,'ServicePanel/servicepanel_page/servicepanel_chatlist_page.html',context)
        else:
            code_check = Codes.objects.filter(user_id=user.id, status=False).count()
            if code_check == 6:
                return redirect('ContactUs:contactus_page')
...
def servicepanel_chat_page(request):
    if request.user.is_authenticated:
        user = Userperson.objects.filter(id=request.user.id).first()
        if user.status == True:
            site_settings = SiteSettings.objects.last()
            ...
            return render(request,'ServicePanel/servicepanel_page/servicepanel_chat_page.html',context)
        else:
            code_check = Codes.objects.filter(user_id=user.id, status=False).count()
            if code_check == 6:
                return redirect('ContactUs:contactus_page')
