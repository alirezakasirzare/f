...
} from '../../components/LazyLoad/constants';
...
export function buildHtml(args: BuildHtmlArgs = {}): string {
  ...
  return `
<div class="splide"${id ? ` id=${id}` : ''}${
    json ? ` data-splide='${json}'` : ''
  }>
...
export function generateSlides(
  ...
  return Array.from<string>({ length }).reduce((html, item, index) => {
    ...
    if (src) {
      imgAttrs.push(
        `src="${URL}/${typeof src === 'string' ? src + '-' : ''}${index}.jpg"`
      );
    }

    if (dataSrc) {
      imgAttrs.push(
        `${SRC_DATA_ATTRIBUTE}="${URL}/${
          typeof dataSrc === 'string' ? dataSrc + '-' : ''
        }${index}.jpg"`
      );
    }
    ...
    if (dataSrcset) {
      imgAttrs.push(
        `${SRCSET_DATA_ATTRIBUTE}="${URL}/${
          typeof dataSrcset === 'string' ? dataSrcset + '-' : ''
        }${index}.jpg 320w"`
      );
    }
